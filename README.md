...
- **LINE Login (OpenID Connect)** — เป็นฟีเจอร์เสริม เพิ่มเติมจาก email/password ไม่ใช่การแทนที่ ผู้ใช้ที่ล็อกอินแล้วสามารถเชื่อมบัญชี LINE ของตัวเองเพื่อให้การแจ้งเตือนการจองส่งไปหา *ตัวเอง* ไม่ใช่แค่แอดมิน เลือกทำเป็นออปชันเสริมแทนที่จะแทนที่ email/password เพราะโจทย์อนุญาตให้เลือก auth method ได้เอง และวิธีเดิมก็ implement + ทดสอบเสร็จแล้ว
...
- **การแจ้งเตือน LINE เป็นแบบ best-effort ไม่ใช่ transactional** การจองจะถูกบันทึกลงฐานข้อมูลก่อน แล้วค่อยยิง LINE push ตามหลัง ถ้ายิงไม่สำเร็จจะแค่ log ไว้ฝั่งเซิร์ฟเวอร์ ไม่ rollback การจอง — การจองไม่ควรหายไปเพียงเพราะ third-party API มีปัญหาชั่วคราว ในฝั่ง create booking นั้น `createBooking` จะ `await` การยิง push จริง (แทนที่จะยิงแล้วไม่รอผล) เพื่อให้ข้อความแจ้งผลลัพธ์บอกความจริงได้ว่าส่งสำเร็จหรือไม่ ไม่ใช่ยืนยันว่าสำเร็จเสมอ (หลักฐานว่าแจ้งเตือนทำงานจริง: ดูสกรีนช็อต/คลิปที่แนบมาพร้อม repo นี้)
...
- ยังไม่มีปุ่ม "Disconnect LINE" หลังจากเชื่อมบัญชีแล้ว (แค่ต้องล้างค่า `lineUserId` แต่ยังไม่ได้ต่อเข้ากับ UI)
